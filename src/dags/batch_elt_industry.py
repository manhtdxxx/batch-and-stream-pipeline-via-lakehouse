...
with DAG(
    ...
    load_to_bronze = SSHOperator(
        ...
        command=(
            f"bash -c '{SPARK_ENV} && {SPARK_SUBMIT} {os.path.join(SPARK_APP, 'bronze', 'raw_industry.py')}'"
        ),
    )
    ...
    process_from_bronze_to_silver = SSHOperator(
        ...
        ssh_conn_id="ssh_spark",
        command=(
            f"bash -c '{SPARK_ENV} && {SPARK_SUBMIT} {os.path.join(SPARK_APP, 'silver', 'processed_industry.py')}'"
        ),
    )
